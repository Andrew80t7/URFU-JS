...
function toBinWhole (whole) {
    return whole.toString(2);
}

function toBinFraction (fractional, lenWholeBin) {
    let fractionalBin = '';
    let flag = (lenWholeBin > 0);
    if (fractional) {
...
function toDecFloat(conversion) {
    ...
    return (1 + mantissa) * Math.pow(2, order) * Math.pow((-1), sign);
}
